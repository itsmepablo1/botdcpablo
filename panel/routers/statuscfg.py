...
class StatusConfig(BaseModel):
    guild_id:                 Any
    status_member_channel_id: Any = None
    status_online_channel_id: Any = None

@router.post("/update")
async def update_status(data: StatusConfig, payload: dict = Depends(verify_token)):
    gid = int(str(data.guild_id))
    updates = {}
    if data.status_member_channel_id:
        updates["status_member_channel_id"] = int(str(data.status_member_channel_id))
    if data.status_online_channel_id:
        updates["status_online_channel_id"] = int(str(data.status_online_channel_id))
    if updates:
